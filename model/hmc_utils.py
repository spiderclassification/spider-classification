...
def preprocess_image_dataset(dataset, datapath, dataset_type, chunk_size=1000):
    ...
    for i in range(0, total_rows, chunk_size):
        ...
        if os.path.exists(os.path.join(datapath, f"X_{dataset_type}_data_chunk{chunk_count}.npy")):
            ...
            chunk_sizes.append(dataset['file_path'].iloc[i:end_index].shape[0])
        else:
            print(f"Preprocessing chunk {chunk_count}..")
            ...
            chunk_sizes.append(chunk_images.shape[0])
            del X_data_mmap
        chunk_count += 1
...
def preprocess_dataset(model_path, data_path, dataset_dir, chunk_size):
    ...
    train_chunk_sizes = preprocess_image_dataset(X_train, data_path, "train", chunk_size=chunk_size)
    ...
    val_chunk_sizes = preprocess_image_dataset(X_val, data_path, "val", chunk_size=len(X_val))
    ...
    return num_classes, num_families, num_genera, num_species, train_chunk_sizes, val_chunk_sizes
